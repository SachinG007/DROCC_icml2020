...
def train(args, 
    ...
    if args.optim == 1:
        optimizer = optim.SGD(model.parameters(),
                                  lr=args.lr,
                                  momentum=args.mom)
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs-only_ce_epochs, eta_min=10e-5)
        print("using SGD")
    else:
    ...
    for epoch in range(args.epochs): 
        ...
        if epoch <= only_ce_epochs:
            adjust_learning_rate1(optimizer,epoch, args)
        # if epoch == only_ce_epochs + 1:
        #     adjust_learning_rate2(optimizer,epoch, args)
        # if epoch == only_ce_epochs:
        #     for param_group in optimizer.param_groups:
        ...
        for j in range(num_iters):
            ...
            if  epoch > only_ce_epochs and args.one_class_adv and torch.sum(target) == args.batch_size:
                ...
                adv_loss_mid, num_adv_pts_mid = one_class_adv_loss(model, data, target,
                                                    args.inp_radius ,device, epoch, grad_list, hidden_adv = 1, step_size=args.step_size)
        ...
        if epoch > args.epochs - 10 and log_recall > max_recall:
        ...
        if (epoch+1) % 1 == 0 :#and epoch > only_ce_epochs - 2:
            # torch.save(model.state_dict(),os.path.join(args.model_dir, 'model_lfoc.pt'))
            print("Precision FPR0.03", log_precision, "Recall FPR0.03", log_recall)
            print("Precision FPR0.05", log_precision5, "Recall FPR0.05", log_recall5)
    #All epochs done
    f = open('follow_fpr_results_lfoc_new.txt','a+')
    f.write(" inp_radius " +str(args.inp_radius) + " step_size " +str(args.step_size) + " inp_lamda " +str(args.inp_lamda)  + " LR " + str(args.lr) +  " Optim " + str(args.optim) +
    " Recall0.03: " + str(max_recall) + " Recall0.05: " + str(max_recall5) + "\n") 
    f.close()  
...
def grad_analyzer(args, 
    ...
    grad = torch.autograd.grad(ce_loss, g_data)[0]
