...
def train(args, 
    ...
    #All epochs done
    f = open('anomaly_kws_results.txt','a+')
    f.write(" inp_radius " +str(args.inp_radius) + " inp_lamda " + str(args.inp_lamda) + " HD " + str(args.hd) + " gamma " + str(args.gamma) +  " Optim " + str(args.optim) + " LR " + str(args.lr) +
...
def test(args, 
    ...
    if epoch > args.epochs - 20 and test_auc > max_auc:
        max_auc = test_auc
        max_auc_epoch = epoch
        # torch.save(model.state_dict(),os.path.join(args.model_dir, 'model_kws.pt'))
