...
parser.add_argument('--optim', type=int, default=0, metavar='N',
                    help='0 : Adam 1: SGD')
parser.add_argument('-dtrain', '--data_path_train', type=str, default='.')  #ROOT DIRECTORY FOR IMAGENET TRAIN DATA
parser.add_argument('-dtest', '--data_path_test', type=str, default='.')  #ROOT DIRECTORY FOR IMAGENET TEST DATA
...
def main():
    dataset = load_dataset("imagenet", args.data_path_train, args.data_path_test, args.normal_class)
    train_loader, test_loader = dataset.loaders(batch_size=args.batch_size)
    model =  models.mobilenet_v2(num_classes=1, width_mult=0.35).to(device)
    model = nn.DataParallel(model)
    #Restore from checkpoint 
    if args.restore == 1:
        model.load_state_dict(torch.load(os.path.join(args.model_dir, 'model_imagenet.pt')))
