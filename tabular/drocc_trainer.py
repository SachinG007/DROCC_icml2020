...
class drocc_trainer():
    ...
    def train(self, learning_rate, ascent_step_size, total_epochs, only_ce_epochs=50, train_loader, val_loader, metric='AUC'):
        for epoch in range(total_epochs): 
            ...
            for data, target, _ in train_loader:
                ...
                if  epoch >= only_ce_epochs and torch.sum(target) == args.batch_size:
                    ...
                    #AdvLoss 
                    adv_loss_inp = self.one_class_adv_loss(data, ascent_step_size)
            ...
            # if epoch > total_epochs - 10:
                # where to declare the best model AUC variable
    ...
    def one_class_adv_loss(x_train_data, ascent_step_size):
        '''
        #Calculate the adversarial loss
        1) Sample points initially at random around the positive training data points
        2) Gradient ascent to find the most optimal point in set N_i(r) classified as +ve (label=0)
           This is done by maximizing the CE loss wrt label 0
        3) Project the points between spheres of radius R and gamma*R (set N_i(r))
